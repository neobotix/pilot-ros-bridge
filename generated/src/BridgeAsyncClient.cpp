...
uint64_t BridgeAsyncClient::vnx_close(const std::function<void()>& _callback, const std::function<void(const vnx::exception&)>& _error_callback) {
...
void BridgeAsyncClient::vnx_purge_request(uint64_t _request_id, const vnx::exception& _ex) {
...
void BridgeAsyncClient::vnx_callback_switch(uint64_t _request_id, std::shared_ptr<const vnx::Value> _value) {
	...
	if(_type_hash == vnx::Hash64(0xa913f47dc68e4876ull)) {
		...
		if(!_result) {
			throw std::logic_error("BridgeAsyncClient: !_result");
		}
		const auto _iter = vnx_queue_vnx_get_config_object.find(_request_id);
		if(_iter != vnx_queue_vnx_get_config_object.end()) {
			...
			throw std::runtime_error("BridgeAsyncClient: received unknown return request_id");
		}
	}
		...
		throw std::runtime_error("BridgeAsyncClient: received unknown return type");
	}
}
